...
void add_read_write_func(){
    ...
    int dim = 0;
    int line = 0;
    insert_Func("read", defined, dim, makeBasicType(0), NULL, line);
    dim = 1;
    struct t_paralist* paralist = (struct t_paralist*) malloc (sizeof(struct t_paralist));
    paralist->para_type = makeBasicType(0);
    paralist->next = NULL;
    insert_Func("write", defined, dim, makeBasicType(0), paralist, line);
}